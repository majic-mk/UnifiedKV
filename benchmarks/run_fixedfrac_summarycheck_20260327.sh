...
LOG=/root/autodl-tmp/.autodl/kv_cache_middleware/benchmarks/run_fixedfrac_summarycheck_20260327.log
run_one() {
  local group="$1"
  local frac="$2"
  local suffix="$3"
  ...
  $PY benchmark_exp1_capacity_table.py \
    ...
    --worker-out "/root/autodl-tmp/.autodl/kv_cache_middleware/benchmarks/${group}_8k_c128_f${suffix}_summaryfix_20260327.json" \
    --worker-step-jsonl "/root/autodl-tmp/.autodl/kv_cache_middleware/benchmarks/${group}_8k_c128_f${suffix}_summaryfix_20260327.steps.jsonl" \
    --worker-step-jsonl-every 1 \
    > "/root/autodl-tmp/.autodl/kv_cache_middleware/benchmarks/${group}_8k_c128_f${suffix}_summaryfix_20260327.log" 2>&1 || true
  echo "=== END ${group} ${frac} $(date '+%F %T') ===" | tee -a "$LOG"
  nvidia-smi --query-gpu=memory.used,memory.free,utilization.gpu --format=csv,noheader | tee -a "$LOG"
  sleep 5
}
run_one main_auto_compress 0.60 060
run_one p2_only_compress 0.50 050
